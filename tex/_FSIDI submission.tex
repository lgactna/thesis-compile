...
which promotes its use in the broader forensic community.

\begin{itemize}
\item
  \textbf{Improved installation, usage, and development}: AKF uses
  modern libraries and approaches to significantly simplify the
  implementation and usage of many of its features, including agent
  communication and disk image modification. It also uses technologies
  such as Vagrant to both reduce the difficulty of setting up AKF and
  improve the reproducibility of datasets created by AKF.
\item
  \textbf{Dataset documentation in both human- and machine-readable
  formats using the CASE ontology}: Many functions throughout AKF
  generate CASE objects, which document well-known artifacts in a
  ...
  by our custom CASE library, which simplifies use of the CASE ontology
  for Python.
\item
  \textbf{Simplified dataset creation using a custom scripting language
  and AI-assisted workflows}: We provide a YAML-based scripting language
  that provides simplified access to the functionality normally exposed
  by our Python libraries. Additionally, we explore the viability of
  generative AI in creating these YAML scripts, allowing datasets to be
  developed from natural language prompts.
\end{itemize}
...
include challenge datasets produced for the DFRWS Forensics Rodeos and
...
demonstrate how AKF improves upon ForTrace's design from a development
perspective.
...
\footnotesize
\centering
\begin{tabularx}{\linewidth}{L{0.25} L{0.375} L{0.375}}
\toprule
...
\midrule
  ...
  function by name through the RPyC connection, as if it were an instance
  method. Routing logic is handled by RPyC. \\
  Sending an object from the VM to the host & Extract the internal state
  of the object and convert it to a string (which may be very difficult);
  ...
  ends. Developers typically do not need to write extra logic to transmit
  complex objects. \\
...
immediately before creating a disk image (as opposed to generating them
as soon as their corresponding artifacts were created), as described in
the prior paragraph.
...
contained in these objects to generate a standalone Markdown document,
which can include formatted text, tables, images, and other visual
elements. The partial documents from each renderer are combined to form
a larger Markdown document (or documents). The combined document can be
converted to a PDF using Pandoc \citep{macfarlanePandoc2025}, a
general-purpose tool for converting between documents of various types.
Users can modify the generated Markdown documents before running Pandoc
if desired.
...
\autoref{fig:figure-2}. Each renderer produces an independent section
that can be included or excluded from reports as desired. So long as the
...
referred to by their aliases, not their fully-qualified module paths
that are also valid names for the \passthrough{\lstinline!module!} key
in the YAML script.
...
\midrule
  ...
  \passthrough{\lstinline!vbox\_init!} & Create an active VirtualBox
  instance bound to a specific virtual machine by name. \\
...
prompts that are related to a corporate espionage scenario. These can
...
explore this, we used Ollama \citep{ollamaOllamaOllama2025} to set up
a local instance of the \passthrough{\lstinline!deepseek-r1:32b!} model,
interacting with it through a locally-hosted instance of Open WebUI
\citep{openwebuiOpenwebuiOpenwebui2025}. DeepSeek-R1 was chosen
because of its comparable performance on various benchmarks to
proprietary LLMs; the 32 billion parameter model was chosen as it was
the largest model with reasonable performance on our hardware.
...
the contents of function docstrings and Pydantic argument models.
Developer-provided docstrings are particularly relevant for both humans
and LLMs, as they contain information about the function's arguments,
preconditions, CASE objects generated, and so on. Although not
...
quality of generated outputs. Additional work is needed to determine the
information that must be provided to an LLM for it to generate
...
throughout digital forensics. Another area of research is in creating
standalone AI-generated artifacts at scale, such as email conversations
and images, that can then be referred to as part of AI-generated
declarative scripts.
...
online LLM is used solely for demonstration; publicly available models
should not be used to analyze confidential datasets.
...
future, we intend to evaluate AKF from the experiences of scenario
developers such as educators (who will be using AKF itself), as well as
the experiences of students (who will be trained on AKF-generated
outputs). We also intend to publish an online version of the internal
API documentation, as well as a user guide.
