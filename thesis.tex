...
the public grew. So did crimes committed with computers; people
discovered that they could hack telephone networks to illegally obtain
...
a some hobbyists and software vendors began to develop dedicated
forensic tools. However, computers had yet to truly enter the
mainstream; most individual computer owners were (relatively wealthy)
hobbyists rather than the general public. In 1984, the U.S. Census
Bureau determined that only about 8.2\% of U.S. households owned a
computer \cite{robertkominskiComputerUseUnited1988}, nearly doubling
to 15\% in 1989 \cite{robertkominskiComputerUseUnited1991}.
...
can collect the Windows prefetch files immediately before a disk image
is created, allowing it to construct CASE prefetch objects that reflect
the disk image without requiring separate tooling. CASE-oriented
behavior can also be implemented in existing subservices; for example,
the \passthrough{\lstinline!chromium!} subservice is capable of creating
...
Markdown tables with the entries shown below, which were then rendered
by Pandoc. (Note that \autoref{fig:scenario-report} uses the Eisvogel
template \cite{waglerWandmalfarbePandoclatextemplate2025} for
Pandoc, significantly improving the appearance and readability of
generated documents. Eisvogel is not distributed with AKF but can be
manually installed alongside Pandoc; \passthrough{\lstinline!akflib!}
will use Eisvogel if it is detected.)
...
Of course, there are other options for generating human-readable (and
...
existing reporting and validation features of AKF. Although not
currently the case, these can be directly integrated into AKF workflows
in the future.
...
scenario. Although synthesizers make it easier to place files at desired
locations or visit websites, these scenario elements must be defined and
created ahead of time. This can be broken down into two separate issues:
...
scripts above is show in \autoref{lst:6.2a}:
...
\passthrough{\lstinline!with!}block. Finally, we create a raw disk image
...
imperative scripts. (In fact, the code in \autoref{lst:6.2a} was largely
derived from the auto-generated code using \autoref{lst:6.3.2a}.)
...
until all modules have been used.
...
\passthrough{\lstinline!vbox\_stop\_machine!} & Power down the currently
active virtual machine. \\
...
While these declarative modules (and the imperative library) provide
users with significant flexibility in \emph{using} AKF, the challenge of
building artifacts and scenarios to generate through these execution
options remains. The remainder of this chapter addresses this challenge.
...
in real-world datasets created as part of a user's benign activity over
time. Such datasets are valuable training material in forensic courses
that encapsulate a long period of study, allowing students to apply many
different techniques learned throughout a curriculum to analyze a
realistic scenario.
...
scenario; in particular, the technical documents ``stolen'' as part of
the scenario are actually files from the general-purpose Govdocs corpora
\cite{garfinkelBringingScienceDigital2009}, with a cover page
denoting its intended role in the scenario. For example, one PowerPoint
file is portrayed as a presentation of the detailed design of the
product, but its actual contents are that of a Yale University
...
analyzing the scenario as a whole. This significantly reduces the time
spent creating individual artifacts, both those directly related to the
focus of the scenario and those that are intended to be background
noise. However, this technique still detracts from the realism of the
scenario, and may not be sufficient to cover all use cases. One notable
example in the Data Leakage Case is the lack of emails on the device;
the sole email conversation is strictly related to the scenario. A
real-world suspect would almost certainly have many other email
conversations that are consistent with their background, such as
employer-related emails or marketing emails from websites that the user
has visited.
...
model to create documents and emails, providing a pipeline through which
thematically-consistent artifacts can be generated and placed onto a
forensic dataset. The same process can be performed for artifacts that
are simply background noise; generating hundreds of ``irrelevant''
emails would be impractical for a human, but viable for an LLM.
...
usage in an AI model -- issues that bring us back to the motivations of
synthesizers described in \autoref{motivation-for-synthetic-datasets}.
...
architecture. However, it may eventually make sense to directly
integrate AI-driven actions into synthesizers, a topic addressed in
\autoref{open-ended-automation-with-ai}.
...
\cite{podellSDXLImprovingLatent2023} in constructing documents and
images, with the goal of demonstrating its viability in scenario
construction. In particular, we focus on building artifacts similar in
quality and theming to those found in the NIST CFReDS Data Leakage Case.
Note that the approach detailed below can be generalized to any
generative model producing text and images.
...
scenario itself. Our goal, then, is to create comparable documents
containing both text and images that are consistent with a theme
provided to the models responsible for generating this content. We
propose a three-step process to generating multimedia artifacts:

\begin{itemize}
\tightlist
\item
  \textbf{Body text}: A prompt is provided to Deepseek-R1 to create
  Markdown document(s) consistent with the scenario. The model is
  instructed to insert suitable image generation prompts (with
  configuration) throughout the document using a special syntax.
\item
  ...
  Markdown syntax for inserting inline images at a known path.
\item
  ...
  approach taken in \textbf{39.5 - Output and validation\#5.3 - Human
  readable reporting}. Pandoc supports various formats, including PDFs,
  Microsoft Word, and Microsoft PowerPoint. Other tools could be used to
  produce artifacts in other formats.
\end{itemize}

The example document in \autoref{fig:artifact-document-example} was
produced by following the steps above on locally-hosted instances of
Deepseek-R1 and SDXL 1.0, using Ollama \cite{OllamaOllama2025} and
ComfyUI \cite{comfyanonymousComfyanonymousComfyUI2025} as
interfaces. An instance of \passthrough{\lstinline!deepseek-r1:14b!} was
...
were generated by instructing Deepseek-R1 to provide a list of suitable
image generation prompts that were related to semiconductor
manufacturing, which were then passed to SDXL 1.0.

Finally, we also noted that the Data Leakage case lacks a significant
number of email conversations. Email conversations as part of a scenario
...
\item
  ...
  coworkers
\item
  Emails that are wholly irrelevant to the scenario, such as marketing
  or spam emails.
\end{itemize}
...
sending/receiving emails, the corporate email domain to use in relevant
emails, and so on. A sample email conversation can be seen in
\autoref{fig:artifact-email-example}.
...
number of synthesizer actions. This is true even if a simplified
declarative syntax is used.

One solution is providing the details of AKF's declarative syntax to an
LLM and instructing it to generate valid declarative scripts. In
...
\item
  ...
  websites.
\item
  The expected inputs for each module is well-defined, as they are
  documented by the Pydantic model that is a required component of each
  module.
\end{itemize}

This information is sufficient for an LLM to ``understand'' the AKF
syntax. Such details could be automatically provided as context to an
LLM through automatically generated documentation, such as the contents
of docstrings and Pydantic model schemas.
