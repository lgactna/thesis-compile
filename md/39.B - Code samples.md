...
The agent's main loop, which receives and interprets these commands, is also straightforward. A simplified version of the main loop is depicted in **!lstb.2.b**:
...
**D-FET** uses a custom language that does not depend on any existing text-based languages [@williamCloudbasedDigitalForensics2011], as seen in the simple scenario in **!lst:b.4.a**:
...
**SFX** uses an XML-based language with tags and attributes that are easily readable [@russellForensicImageDescription2012], as shown in **!lst:b.4.c**:
...
Finally, the work of Yannikos et al. is particularly notable because it appears to be fully GUI-based, expecting users to visually construct Markov chains to define scenarios [@yannikosDataCorporaDigital2014]. An example scenario from their publication can be seen in Figure **!fig:yannikos-gui**:
