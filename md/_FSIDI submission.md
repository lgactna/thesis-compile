...
Many early datasets were derived from real sources, such as the used hard drives collected by Garfinkel from 1998 to 2006, the Enron email corpus obtained during the federal investigation of Enron, the public Apache mailing archives, and facial recognition collections [@garfinkelForensicCorporaChallenge2007; @grajedaAvailabilityDatasetsDigital2017; @yannikosDataCorporaDigital2014; @ricanekMORPHLongitudinalImage2006]. There were also some synthetic datasets developed during this period, such as the standalone test datasets created by NIST [@woodsCreatingRealisticCorpora2011]. More recent examples include challenge datasets produced for the DFRWS Forensics Rodeos and vendor capture-the-flag (CTF) competitions, such as Cellebrite CTF and Magnet CTF.
...
**!tbl:akf-comparison** provides brief examples of use cases that demonstrate how AKF improves upon ForTrace's design from a development perspective.

*!tbl:akf-comparison|Comparison of the work required to implement certain actions between ForTrace and AKF.|0.25,0.375,0.375*

| Use case                                                                     | Implementation in ForTrace                                                                                                                                                                                                 | Implementation in AKF                                                                                                                                             |
| ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Calling an exposed function in the VM from the host                          | Manually build a valid string containing the name of the function and its serialized parameters; implement deserialization and routing logic to correctly import and call the corresponding function in the VM.            | Call the function by name through the RPyC connection, as if it were an instance method. Routing logic is handled by RPyC.                                        |
| Sending an object from the VM to the host                                    | Extract the internal state of the object and convert it to a string (which may be very difficult); manually implement deserialization logic that will correctly re-instantiate the object based on the extracted state.    | All serialization logic is already automatically handled by RPyC on both ends. Developers typically do not need to write extra logic to transmit complex objects. |
...
AKF implements human-readable reporting through a set of "renderers." Each renderer accepts a complete CASE bundle and extracts all CASE objects of a particular type. The renderer then uses the information contained in these objects to generate a standalone Markdown document, which can include formatted text, tables, images, and other visual elements. The partial documents from each renderer are combined to form a larger Markdown document (or documents). The combined document can be converted to a PDF using Pandoc [@macfarlanePandoc2025], a general-purpose tool for converting between documents of various types. Users can modify the generated Markdown documents before running Pandoc if desired.

A single CASE bundle can be passed through as many or as few renderers as needed to generate a suitable report for a dataset, as depicted in **!fig:figure-2**. Each renderer produces an independent section that can be included or excluded from reports as desired. So long as the original CASE bundle is available, users can reanalyze datasets with arbitrary renderers; this means that dataset reports can be regenerated with as much detail as a user needs for a specific use case. Furthermore, if new renderers are developed for artifacts that are present in older datasets, the human-readable report can be regenerated to include these artifacts. 
...
| `vbox_init`              | Create an active VirtualBox instance bound to a specific virtual machine by name.                                 |
...
In the same way that declarative scripts are an abstraction around imperative code, an LLM can be used to facilitate natural language prompts as an abstraction around creating declarative scripts. To explore this, we used Ollama [@ollamaOllamaOllama2025] to set up a local instance of the `deepseek-r1:32b` model, interacting with it through a locally-hosted instance of Open WebUI [@openwebuiOpenwebuiOpenwebui2025]. DeepSeek-R1 was chosen because of its comparable performance on various benchmarks to proprietary LLMs; the 32 billion parameter model was chosen as it was the largest model with reasonable performance on our hardware.
...
We observed that this information was generally sufficient for DeepSeek-R1 to "understand" the AKF syntax, allowing it to generate simple declarative scripts. Although the prompt containing this information was manually written, much of the prompt could be used as a template and substituted with the contents of automatically generated documentation. For example, the table of available modules and their descriptions could be generated through existing code metadata, such as the contents of function docstrings and Pydantic argument models. Developer-provided docstrings are particularly relevant for both humans and LLMs, as they contain information about the function's arguments, preconditions, CASE objects generated, and so on. Although not implemented, this use of existing internal documentation is expected to provide the necessary context for an LLM to function in the future.
...
Further research is needed to identify common pitfalls when generating AKF scenarios and individual artifacts through AI, as well as best practices for similar research- and education-related AI workflows throughout digital forensics. Another area of research is in creating standalone AI-generated artifacts at scale, such as email conversations and images, that can then be referred to as part of AI-generated declarative scripts.
