...
This chapter addresses the modules responsible for allowing users to invoke the framework through a standard Python script and a high-level YAML file. It also addresses the generative AI modules that can assist a user in building a scenario, as depicted in the partial architectural diagram below.

!**39.6 - Building scenarios 2025-02-08 17.23.40.excalidraw**

At this point, we have provided the implementations for automating artifact generation in a near-deterministic manner with comprehensive logging and reporting. However, there is still the challenge of exposing this functionality in a user-friendly manner. At a high level, there are two primary ways to define an input to a synthesizer:
...
Consider the following declarative SFX code taken from Russell et al. shown in **!lst:6.1** [@russellForensicImageDescription2012]:

```xml
!lst:6.1|Sample SFX scenario code
<disk>
...
The same might be expressed in ForTrace [@gobelForTraceHolisticForensic2022] as shown in **!lst:6.2**, excluding additional code required for ground truth and disk image generation:

```python
!lst:6.2|Sample ForTrace scenario code [@gobelForTraceHolisticForensic2022]
import logging
...
ForTrace [@gobelForTraceHolisticForensic2022] uses YAML to express scenarios in a declarative manner. ForTrace significantly influenced both the AKF declarative syntax itself and the implementation of the declarative interpreter, primarily because it was the sole synthesizer with both imperative and declarative support known to be open-source. 
...
The following is a simple Ansible playbook with a single play, derived from the official Ansible documentation for playbooks [@ansibleprojectcontributorsAnsiblePlaybooks]:
